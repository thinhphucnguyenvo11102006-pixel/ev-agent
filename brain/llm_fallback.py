...
class LLMFallbackChain:
    ...
    async def chat_stream(self, messages, tools=None, temperature=0.7, max_tokens=2048):
        """Stream chat with fallback (falls back to non-stream on failure)."""
        try:
            async for chunk in self.active_client.chat_stream(
                messages=messages,
                tools=tools,
                temperature=temperature,
                max_tokens=max_tokens,
            ...
            logger.warning(f"Streaming failed on {self.active_client.name}: {e}")
